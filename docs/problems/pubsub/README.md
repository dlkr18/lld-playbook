...
classDiagram
    class PubSubService {
        ...
        +subscribe(topicId, subscriptionId, filter) String
        +unsubscribe(subscriptionId)
        +publish(topicId, message)
        +pull(subscriptionId, maxMessages) List~Message~
        +acknowledge(subscriptionId, ackId)
    }
    ...
    class InMemoryPubSubService {
        ...
        +createTopic(name) String
        +publish(topicId, message)
        +subscribe(topicId, subscriptionId, filter)
        +pull(subscriptionId, maxMessages)
    }
    ...
    class Subscription {
        ...
        -Map~String,Message~ pendingAcks
        +enqueue(Message)
        +pull(maxMessages) List~Message~
        +acknowledge(ackId)
    }
    ...
    InMemoryPubSubService --> Topic
    InMemoryPubSubService --> Subscription
    Subscription --> Message
    Subscription --> MessageFilter
    MessageFilter <|.. AttributeFilter
    MessageFilter <|.. CompositeFilter
